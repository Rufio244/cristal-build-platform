...
app.post('/api/auth/register', async (req, res) => {
  ...
  } catch (error) { res.status(error.code === 'P2002' ? 409 : 500).json({ error: error.code === 'P2002' ? 'อีเมลนี้ถูกใช้งานแล้ว' : error.message }); }
});
...
app.post('/api/ai/generate', auth, async (req, res) => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (charged.count !== 1) throw Object.assign(new Error('AI Credits หมดแล้ว'), { status: 400 });
      ...
      return { website, aiCredits: user.aiCredits - 1 };
    });
...
app.patch('/api/websites/:id', auth, async (req, res) => {
  const website = await prisma.website.updateMany({ where: { id: req.params.id, userId: req.userId }, data: { ...(req.body.title && { title: req.body.title }), ...(req.body.pages && { pages: req.body.pages }), ...(req.body.status && { status: req.body.status }) } });
  if (!website.count) return res.status(404).json({ error: 'ไม่พบเว็บไซต์' });
  res.json({ success: true });
});

app.post('/api/websites/:id/publish', auth, async (req, res) => {
  const website = await prisma.website.updateMany({ where: { id: req.params.id, userId: req.userId }, data: { status: 'PUBLISHED' } });
  if (!website.count) return res.status(404).json({ error: 'ไม่พบเว็บไซต์' });
...
app.post('/api/billing/checkout', auth, async (req, res) => {
  ...
  res.json({ success: true, mode: 'local-demo', plan: user.plan, aiCredits: user.aiCredits, message: 'อัปเกรดแพ็กเกจในโหมดทดลองสำเร็จ' });
});
